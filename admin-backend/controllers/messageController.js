...
exports.sendMessage = async (req, res) => {
  ...
  try {
    ...
    if (templateId) {
      const template = await MessageTemplate.findOne({ _id: templateId, organizationId: orgId });
...
exports.getMessageStatus = async (req, res) => {
  try {
    const orgId = req.session.orgId;
    const message = await ScheduledMessage.findOne({ _id: req.params.id, organizationId: orgId });
    if (!message) {
...
exports.runCronJob = async (req, res) => {
  try {
    const { processScheduledMessages } = require('../helpers/messageScheduler');
