      ...
      <div className="max-w-[1600px] mx-auto px-6 sm:px-8 lg:px-12">
        <div className="flex justify-between items-center h-16  rounded-2xl px-6">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            <span onClick={() => navigate('/features')} className="cursor-pointer text-sm font-bold text-slate-600 hover:text-[#3fc07c] transition-colors">{t('nav.features')}</span>
            <span onClick={() => navigate('/pricing')} className="cursor-pointer text-sm font-bold text-slate-600 hover:text-[#3fc07c] transition-colors">{t('nav.pricing')}</span>
            <span onClick={() => navigate('/contact')} className="cursor-pointer text-sm font-bold text-slate-600 hover:text-[#3fc07c] transition-colors">{t('nav.contact')}</span>
          </nav>

          {/* Right Actions */}
          <div className="flex items-center gap-4 sm:gap-6">
            {/* Language Switcher */}
            <div className="flex items-center cursor-pointer gap-1.5 text-xs font-black text-slate-400">
              <span className={`hover:text-slate-900 transition-colors ${lang === 'EN' ? 'text-slate-900' : ''}`} onClick={() => setLang('EN')}>EN</span>
              <span className="text-slate-200">|</span>
              <span className={`hover:text-slate-900 transition-colors ${lang === 'HI' ? 'text-slate-900' : ''}`} onClick={() => setLang('HI')}>HI</span>
            </div>
              ...
              <div className="flex items-center gap-3">
                {isLoginPage ? (
                  <Button
                    ...
                    variant="outline"
                    className="border-2 border-slate-100 hover:bg-slate-50 text-slate-900 rounded-xl font-bold px-6"
                  >
                    Sign Up
                  </Button>
                ) : (
                  <>
                    <Button
                      ...
                      className="bg-[#3fc07c] hover:bg-[#2da05f] text-white px-7 rounded-xl font-bold shadow-lg shadow-green-200 transition-all active:scale-95"
                    >
                      {t('nav.login')}
                    </Button>
                    {!isSignupPage && (
                      <Button
                        onClick={() => navigate('/signup')}
                        variant="ghost"
                        className="hidden sm:flex font-bold text-slate-600"
                      >
                        Sign Up
                      </Button>
                    )}
