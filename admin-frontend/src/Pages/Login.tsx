...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await login(adminPhone, password);
    } catch (err: any) {
      ...
      <div className="mt-15 fade-in animate-in flex items-center justify-center p-4 relative overflow-hidden">
        <div className="bg-white rounded-3xl shadow-[0_15px_60px_-15px_rgba(0,0,0,0.1)] w-full max-w-[420px] p-8 sm:p-10 border border-slate-100 relative z-10">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-[1.75rem] font-black text-[#111928] tracking-tight">Log In to Invitely</h1>
            <Whatsapp h={10} w={10} />
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-1.5">
              <Label htmlFor="phone" className="text-[15px] font-medium text-slate-700">WhatsApp Number</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="password" className="text-[15px] font-medium text-slate-700">System Password</Label>
            ...
            <div className="flex items-center space-x-3 mt-4">
              ...
              <label htmlFor="keep-logged-in" className="text-[14px] font-medium leading-none text-slate-700 cursor-pointer">
                Keep me logged in for rapid extraction
              </label>
            </div>
          </form>
        </div>
