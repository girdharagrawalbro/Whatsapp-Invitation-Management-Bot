...
import { fetchUsers, addUser, editUser, deleteUser, bulkDeleteUsers } from '../services/apiService';
...
export default function ManageUsers() {
  ...
  const queryError = error ? 'Failed to load users' : null;
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    editUserMutation.mutate({ id: currentUser._id, data: formData });
  }

  const handleDeleteUser = (user: User) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      deleteUserMutation.mutate({ id: user._id, role: user.role });
    }
  }
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, activeTab === 'admin' ? "Admin Users" : "Normal Users");
    XLSX.writeFile(workbook, `${activeTab}_users_export.xlsx`);
  ...
  const filteredUsers = users.filter(
    user => {
      ...
      const matchesRole = activeTab === 'admin' ? user.role === 'admin' : (user.role !== 'admin');
      ...
      <Card className="m-6 border-none shadow-sm">
        <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Manage admin users and normal application contacts.</p>
          </div>
        ...
        <CardContent>
          <div className="flex border-b border-slate-200 mb-6">
            <button
              onClick={() => { setActiveTab('normal'); setCurrentPage(1); setSelectedUsers([]); }}
              className={`pb-3 px-6 text-sm font-semibold border-b-2 transition-all ${activeTab === 'normal'
                ? 'border-[#3fc07c] text-[#3fc07c]'
            ...
            <button
              onClick={() => { setActiveTab('admin'); setCurrentPage(1); setSelectedUsers([]); }}
              className={`pb-3 px-6 text-sm font-semibold border-b-2 transition-all ${activeTab === 'admin'
                ? 'border-[#3fc07c] text-[#3fc07c]'
                : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
            >
          ...
          {queryError ? (
            <div className="text-destructive p-8 text-center bg-destructive/10 rounded-lg">{queryError}</div>
          ) : loading ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/50">
                    ...
                    {activeTab === 'normal' && <TableHead>Type</TableHead>}
                  ...
                  {currentUsers.length > 0 ? (
                    currentUsers.map((user) => (
                      <TableRow key={user._id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <TableCell>
                          <Badge className={`capitalize ${user.role === 'admin' ? 'bg-[#3fc07c] hover:bg-[#3fc07c] text-white' : 'bg-slate-100 text-slate-800'}`}>
                            {user.role || 'user'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <div className="text-xs text-muted-foreground">
                            {user.lastInteraction || user.lastActiveAt 
                              ? new Date(user.lastInteraction || user.lastActiveAt).toLocaleString() 
                              : 'Never'}
                          </div>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => handleDeleteUser(user)} className="h-8 w-8 text-destructive">
                    ...
                    <TableRow>
                      <TableCell colSpan={activeTab === 'normal' ? 7 : 6} className="h-24 text-center text-muted-foreground">
                        No users found.
                      </TableCell>
                    </TableRow>
                  )}
              ...
              <CardContent className="space-y-4">
                ...
                {formData.role === 'admin' && (
                  <div className="grid gap-2 animate-in fade-in slide-in-from-top-1">
                    <label className="text-sm font-medium text-[#3fc07c] font-bold">Admin Dashboard Password</label>
                    <Input
                      type="password"
                      required={showAddForm && formData.role === 'admin'}
                      placeholder={showAddForm ? "Enter admin login password" : "Leave blank to keep current password"}
